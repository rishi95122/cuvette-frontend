...
function YourJobs() {
    ...
    try {
      if (user) {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_BACKEND_URI}/api/jobs/${user._id}`,
          { headers }
        );
      ...
      setLoading(false);
      toast.error(
        error.response?.data?.error || "An error occurred while fetching jobs."
      );
    }
  };
  ...
  if (loading)
    return (
      <div className="grid gap-6 md:grid-cols-2 mt-20 lg:grid-cols-3">
        <YourjobsSkeltons />
        <YourjobsSkeltons />
        <YourjobsSkeltons />
      </div>
